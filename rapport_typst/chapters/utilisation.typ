...
Pour vérifier la version de GTK 3 (commande variable selon la distribution) :
```bash
gtk-launch --version
# ou
dpkg -l libgtk-3-0
...
Téléchargez le fichier `JSmithFX-1.0-SNAPSHOT.jar` depuis la page des "Releases" du projet. Actuellement, pour des soucis de discussions par rapport à des licences, le projet est en privé. Dans le cadre de ce rapport, l'exécutable est fourni dans les annexes avec le code.

==== Lancement de l'application

La méthode recommandée est d'utiliser le terminal pour voir les éventuels messages d'erreur au démarrage. Pour ce faire, on ouvre un terminal dans le dossier du fichier `.jar` et on exécute :
...
Si Java n'est pas installé ou si la version est trop ancienne, il faut télécharger et installer le JRE 25 (version LTS recommandée par rapport au JRE 23) :
...
Après installation, on peut vérifier la version installée :
```bash
java -version
```
...
Le point de charge (LD) est déjà affiché sur l'abaque. On peut commencer à ajouter des composants directement sans créer de nouveau projet.
...
Pour repartir de zéro et effacer tous les composants ajoutés :
...
_Attention : Cette opération efface le circuit en cours. Il faut penser à sauvegarder le projet avant si nécessaire._

=== Ajouter des composants

Il existe deux méthodes pour construire le circuit d'adaptation :
...
3. Déplacez la souris sur l'abaque. La trajectoire possible du composant s'affiche en temps réel ainsi que la valeur calculée dans le panneau de droite.
4. Cliquez sur l'abaque pour valider la position. La valeur du composant est calculée automatiquement et le composant est ajouté au schéma du circuit électrique.
5. Pour quitter ce mode d'ajout à la souris, appuyez sur `ESC`.
...
On peut insérer un composant à un endroit précis du circuit :
...
_Note : Pour les lignes de transmission, il faut spécifier l'impédance caractéristique ($Z_0$) de la ligne et sa permittivité ($epsilon_r$)._
...
Lorsqu'on clique sur un composant dans le schéma électrique, le panneau "Add Component" devient "Modify Component" :
1. On peut complètement changer le type du composant (passer d'un condensateur à une inductance, par exemple).
2. On peut changer sa position (série ↔ parallèle).
...
Lorsqu'on déplace la souris sur l'abaque, le panneau "Cursor Values" du côté droit de l'application affiche les valeurs à la position de la souris en temps réel :
...
2. Sélectionnez le fichier `.s1p` (Format Touchstone).
3. Le panneau de gestion S1P s'ouvre à droite.
4. On peut activer jusqu'à 3 filtres de fréquences pour mettre en évidence des bandes spécifiques (*Set > Enable X Frequency Filter*).
5. (Optionnel) Cochez *"As Load"* pour utiliser le fichier S1P comme charge complexe variant avec la fréquence lors des simulations. C'est extrêmement utile pour visualiser en direct les changements apportés par le circuit d'adaptation aux mesures fournies par le fichier S1P.
...
2. Dans la fenêtre qui s'affiche, on peut choisir les cercles de VSWR qu'on veut afficher parmi une sélection de valeurs. Cliquez sur *OK* pour que les cercles s'affichent sur l'abaque.
...
1. En haut de l'interface, au-dessus du schéma électrique, on trouve un menu déroulant avec les différents indices des circuits du projet suivi de boutons *New*, *Copy* et *Del*.
...
Pour travailler avec des composants réels qu'on possède physiquement :

1. Menu *Use discrete components > Configure discrete components*.
2. Dans la fenêtre, ajoutez les composants disponibles :
...
3. On peut importer/exporter le catalogue au format CSV (boutons *Import* et *Export*).
4. Fermez la fenêtre et activez *Use discrete components* dans le menu du même nom.
5. Lors de l'ajout à la souris, des petits points cyan apparaissent sur l'abaque indiquant exactement où se trouvent les composants disponibles. La valeur se "magnétise" automatiquement sur le composant réel le plus proche.

=== Undo / Redo

On peut annuler et refaire les dernières actions :
...
4. Pour sauvegarder un projet déjà existant dans un nouveau fichier, on peut utiliser *Save As*.

*Charger :*
1. Menu *File > Open*
2. Sélectionnez le fichier `.jsmfx`
3. Le projet se charge avec tous ses circuits.

_Note : Les fichiers S1P importés et les sweeps effectués ne sont pas sauvegardés dans le projet. Seuls les circuits et leurs paramètres le sont. On peut exporter les sweeps séparément en S1P si besoin._
